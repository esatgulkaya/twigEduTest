...
describe('Util - groupArrayElements test', () => {
    ...
    test('Array with zero divider, should throw error', () => {        
        expect(()=> groupArrayElements([1, 2, 3, 4, 5,6], 0)).toThrow(CHUCK_COUNT_PARAM_ERROR);
    })

    test('Array with negative divider, should throw error', () => {        
        expect(()=> groupArrayElements([1, 2, 3, 4, 5,6], -3)).toThrow(CHUCK_COUNT_PARAM_ERROR);
    })

    test('Array with non-integer divider, should throw error', () => {        
        expect(()=> groupArrayElements([1, 2, 3, 4, 5,6], 1.3)).toThrow(CHUCK_COUNT_PARAM_ERROR);
    })
})