...
export default function groupArrayElements(
  arrayToBeDivided: Array<string | number>,
  chunkCount: number
) {
  if (chunkCount <= 0 || !Number.isInteger(chunkCount)) {
    throw new Error(CHUCK_COUNT_PARAM_ERROR);
  }
  const chunkMaxLength: number = Math.round(
    arrayToBeDivided.length / chunkCount
  );

  const dividedArray: Array<Array<string | number>> = [];
  for (let i = 0; i < arrayToBeDivided.length; i += chunkMaxLength) {
    // Creates a slice from input array to create new chunk
    const nextChunk: Array<string | number> = arrayToBeDivided.slice(
      i,
      i + chunkMaxLength
    );
